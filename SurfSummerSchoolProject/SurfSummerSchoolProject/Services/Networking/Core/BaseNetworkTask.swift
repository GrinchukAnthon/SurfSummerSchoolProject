...
extension BaseNetworkTask where Input == EmptyModel {
    ...
    func performRequest(_ onResponseWasReceived: @escaping (_ result: Result<AbstractOutput, Error>) -> Void) {
        performRequest(EmptyModel(), onResponseWasReceived)
    }
}
...
private extension BaseNetworkTask {
    ...
    func getRequest(with paremeters: AbstractInput) throws -> URLRequest {
        ...
        switch method {
        case .get:
            let newUrl = try getUrlWithQueryParameters(for: url, parameters: paremeters)
            request = URLRequest(url: newUrl)
        case .post:
            request = URLRequest(url: url)
            request.httpBody = try getParametersForBody(from: paremeters)
        }
    ...
    func getUrlWithQueryParameters(for url: URL, parameters: AbstractInput) throws -> URL {
        ...
        guard let parametersInDictionaryRepresentation = parametersInDictionaryRepresentation as?
                [String: Any] else {
